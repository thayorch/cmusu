<template>
  <Navbar />
  <section
    class="relative z-10 max-w-7xl mx-auto px-6 pt-10 pb-24 lg:pb-20 animate-fade-in"
  >
    <!-- Page Header -->
    <div class="text-center mb-10">
      ...
      <h2 class="font-black text-dark text-4xl md:text-5xl mb-3">
        ครุภัณฑ์ส่วนกลาง
      </h2>
    ...
    <div v-else class="flex flex-col lg:flex-row gap-6 items-start">
      ...
      <div class="flex-1 min-w-0">
        <!-- User bar -->
        <div
          class="flex justify-between items-center mb-6 glass p-3.5 px-5 rounded-2xl shadow-sm"
        >
          <div class="flex items-center gap-3">
            <div
              class="w-9 h-9 bg-gradient-to-br from-[#a259ff] to-[#5bc8ff] text-white rounded-full flex items-center justify-center font-bold"
            >
        ...
        <div v-if="isLoading" class="text-center py-20 text-gray-400">
          <div class="inline-flex flex-col items-center gap-3">
            <div
              class="w-7 h-7 border-4 border-[#a259ff]/20 border-t-[#a259ff] rounded-full animate-spin"
            />
        ...
        >
          ...
          >
            ...
            <div class="p-5 flex-1 flex flex-col">
              <h3
                class="font-black text-gray-900 text-base mb-1.5 line-clamp-1"
              >
                {{ item.name }}
              </h3>
              <p
                class="text-xs text-gray-400 mb-5 flex-1 line-clamp-2 leading-relaxed"
              >
      ...
      <div class="hidden lg:block w-[380px] shrink-0 sticky top-6">
        <div
          class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden flex flex-col"
        ...
        >
          ...
          <div class="px-5 py-4 border-b border-gray-100 shrink-0">
            <div class="flex items-center gap-3">
              <div
                class="w-9 h-9 bg-[#a259ff]/10 rounded-xl flex items-center justify-center shrink-0"
              >
                <ShoppingCartIcon class="w-5 h-5 text-[#a259ff]" />
              </div>
              <div>
                <h3 class="font-black text-gray-900 text-base leading-tight">
                  ตะกร้ายืมครุภัณฑ์
                </h3>
                <p class="text-xs text-gray-400 mt-0.5">
                  <template v-if="cart.length > 0"
                    >{{ cart.length }} รายการ ·
                    {{ totalCartQty }} ชิ้น</template
                  >
          ...
          <div class="overflow-y-auto flex-1 px-5 py-4 space-y-3">
            ...
            >
              ...
              <p class="text-xs text-gray-300 mt-1">
                เลือกอุปกรณ์จากแคตตาล็อกทางซ้าย
              </p>
            </div>
            <TransitionGroup
            ...
            >
              ...
              >
                ...
                />
                <div class="flex-1 min-w-0">
                  <p class="font-bold text-xs text-gray-900 line-clamp-1">
                    {{ cartItem.equipment.name }}
                  </p>
                  <p class="text-[10px] text-gray-400 mt-0.5">
                    คงเหลือ {{ cartItem.equipment.available_quantity }} ชิ้น
                  </p>
                </div>
                <div
                  class="flex items-center bg-white border border-gray-200 rounded-xl overflow-hidden shrink-0"
                >
                  <button
                    @click="updateCartQty(index, -1)"
                    class="w-7 h-7 flex items-center justify-center text-gray-500 hover:bg-gray-100 font-bold transition-colors"
                  >
                    −
                  </button>
                  <input
                    ...
                    type="number"
                    class="w-9 text-center text-sm font-black outline-none bg-transparent"
                  />
                  <button
                    @click="updateCartQty(index, 1)"
                    class="w-7 h-7 flex items-center justify-center text-gray-500 hover:bg-gray-100 font-bold transition-colors"
            ...
            <div class="pt-3 border-t border-gray-100">
              <p
                class="text-[10px] font-black uppercase tracking-wider text-gray-400 mb-3"
              >
                ข้อมูลผู้ยืม
              </p>
              <form
                id="borrow-form-desktop"
              ...
              >
                <div class="grid grid-cols-2 gap-2.5">
                  <div>
                    <label
                      class="block text-[11px] font-bold mb-1 text-gray-500"
                    ...
                    <input
                      ...
                      class="w-full px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 focus:bg-white focus:ring-2 focus:ring-[#a259ff]/30 focus:border-[#a259ff] outline-none text-xs transition-all"
                    />
                  </div>
                  <div>
                    <label
                ...
                <div>
                  <label class="block text-[11px] font-bold mb-1 text-gray-500"
                ...
                <div>
                  <label class="block text-[11px] font-bold mb-1 text-gray-500"
                    >สังกัดคณะ/วิทยาลัย/หน่วยงาน/ชมรม
                ...
                <div>
                  ...
                  >
                  <input
                    v-model="formData.purpose_location"
                    type="text"
                    required
      ...
      >
        ...
        >
          ...
          >
            <!-- Drag handle + close -->
            <div
              class="flex justify-center items-center pt-3 pb-2 shrink-0 relative"
            >
            ...
            <div class="overflow-y-auto flex-1 px-5 py-4 space-y-3">
              ...
              >
                ...
                <p class="text-sm font-bold text-gray-400">ตะกร้าว่างเปล่า</p>
                <p class="text-xs text-gray-300 mt-1">
                  ปิดแล้วเพิ่มอุปกรณ์จากแคตตาล็อก
                </p>
              </div>
              <div
                v-for="(cartItem, index) in cart"
              ...
              >
                ...
                >
                  <button
                    @click="updateCartQty(index, -1)"
                    class="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-gray-100 font-bold transition-colors"
                  >
                    −
                  </button>
                  <input
                    ...
                    @change="validateCartQty(index)"
                    type="number"
                    class="w-10 text-center text-sm font-black outline-none bg-transparent"
                  />
                  <button
                    @click="updateCartQty(index, 1)"
                    class="w-8 h-8 flex items-center justify-center text-gray-500 hover:bg-gray-100 font-bold transition-colors"
...
const validateCartQty = (index) => {
  ...
  if (item.quantity > item.equipment.available_quantity) {
    ...
    showAlert(
      `ปรับจำนวนสูงสุดได้ที่ ${item.equipment.available_quantity} ชิ้น`,
      "warning",
    );
...
const submitBorrowCheckout = async () => {
  ...
  try {
    ...
    if (res.ok) {
      await showAlert(
        "ส่งคำร้องสำเร็จ! ระบบตัดยอดอุปกรณ์เรียบร้อยแล้ว",
        "success",
      );
      cart.value = [];
